...
serve(async (req) => {
  ...
  try {
    ...
    const fbPostId = post.published_fb_id;
    const caption = post.title || post.content || "";
    const postType = post.post_type || "text";

    // Facebook Graph API update: use POST to /{post-id}
    // For text posts: update `message`
    // For photo posts: the photo post ID from /photos is a photo object, update caption via message
    const updateBody: Record<string, string> = { access_token: pageToken };

    if (postType === "text") {
      updateBody.message = caption;
    } else if (postType === "image") {
      // Photo posts returned from /photos give a photo ID; we can update caption
      updateBody.message = caption;
    } else if (postType === "video" || postType === "reel") {
      updateBody.description = caption;
    }

    const res = await fetch(`https://graph.facebook.com/v22.0/${fbPostId}`, {
      method: "POST",
    ...
    if (fbResult.error) {
      throw new Error(`Facebook update error: ${fbResult.error.message}`);
    }
